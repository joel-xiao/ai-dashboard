...
export default {
  id: 'datasource-crash-error-states--v2',
  ...
  requester: async function(args, metrics) {
    ...
    if (!metric_params.dimensions.includes('type') && metric_params.dimensions.includes('name')) {
      ...
      if (ret.result === 'ok') {
        ...
        if (metric_params.dimensions.some( metric =>  metric === 'error_label')) {
          for(let i = 0; i < ret.data.length; i++) {
            ...
            let raw = await appService.getAppCrashStats(appid, opt);
