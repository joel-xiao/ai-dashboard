...
export default {
  ...
  requester: async function(args, metrics, query) {
    ...
    let ret = await aiopsService.getHealthScore(options);
    if (ret.result === 'ok') {
      if (params.group_by) {
        let group_by = ret.data.map( r => {
          let score = r.score || 0;
          score = parseInt(score);
          return {
            ...r,
            names: ['score'],
            data: [ score ? Number(score.toFixed(2)) : 0 ],
          }
        })
        result = { 
          default: {},
